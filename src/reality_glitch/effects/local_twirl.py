...
class LocalTwirlEffect(BaseEffect):
    definition = EffectDefinition(EffectId.LOCAL_TWIRL, "סחרור מקומי")

    def apply(self, image: Image.Image, settings: dict[str, Any]) -> Image.Image:
        strength = float(settings.get("strength", 3.0))
        center_x_pct = float(settings.get("center_x", 50.0))
        center_y_pct = float(settings.get("center_y", 45.0))
        radius_pct = float(settings.get("radius", 30.0))
        falloff = float(settings.get("falloff", 1.6))
    ...
    def settings_for_frame(self, settings: dict[str, Any], progress: float, frame_index: int) -> dict[str, Any]:
        ...
        frame_settings["strength"] = float(frame_settings.get("strength", 3.0)) * progress
