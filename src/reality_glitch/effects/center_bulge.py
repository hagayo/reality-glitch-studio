...
class CenterBulgeEffect(BaseEffect):
    definition = EffectDefinition(EffectId.CENTER_BULGE, "ניפוח מרכזי")
    ...
    def apply(self, image: Image.Image, settings: dict[str, Any]) -> Image.Image:
        ...
        scale = 1.0 / (1.0 + (strength * influence))
    ...
    def settings_for_frame(self, settings: dict[str, Any], progress: float, frame_index: int) -> dict[str, Any]:
        ...
        frame_settings["strength"] = float(frame_settings.get("strength", 0.65)) * progress
